    ...
    ynabAPI.budgets
      ...
      .then((budgetsResponse) => {
        ...
        ynabBudgetId = config.budgetId
          ? config.budgetId
          : budgetsResponse.data.budgets[0].id;
        console.log(
          "🚀 ~ file: node_helper.js ~ line 28 ~ ynabAPI.budgets.getBudgets ~ ynabBudgetId",
          ynabBudgetId
        );
    ...
    ynabAPI.categories
      ...
      .then((categoriesResponse) => {
        const map = []
          .concat(
            ...Array.from(
              categoriesResponse.data.category_groups.map((a) =>
                Array.from(a.categories)
              )
            )
          )
          .reduce((map, o) => {
